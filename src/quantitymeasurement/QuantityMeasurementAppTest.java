...
class QuantityMeasurementAppTest {
	...
	void testTemperatureEquality_SymmetricProperty() {
	    ...
	    Quantity<TemperatureUnit> t2 = new Quantity<>(32.0, TemperatureUnit.FAHRENHEIT);
	    assertTrue(t1.equals(t2));
	    assertTrue(t2.equals(t1));  // symmetric
	}

	@Test
	void testTemperatureEquality_DifferentValues_NotEqual() {
	...
	@Test
	void testTemperatureEquality_NullUnit() {
	    assertThrows(IllegalArgumentException.class, () -> {
	        new Quantity<>(0.0, (TemperatureUnit) null);
	    });
	}
	...
	void testTemperatureConversion_CelsiusToFahrenheit_100() {
	    ...
	    assertEquals(212.0, result.convertToBaseUnit(), EPSILON);  // base = Celsius, so 212F→100C
	}
	...
	@Test
	void testTemperatureConversion_CelsiusToFahrenheit_Negative40() {
	    Quantity<TemperatureUnit> t = new Quantity<>(-40.0, TemperatureUnit.CELSIUS);
	    Quantity<TemperatureUnit> result = t.convertTo(TemperatureUnit.FAHRENHEIT);
	    assertEquals(-40.0, result.convertToBaseUnit(), EPSILON);  // -40F → -40C
	}

	@Test
	void testTemperatureConversion_SameUnit_NoChange() {
	    Quantity<TemperatureUnit> t = new Quantity<>(50.0, TemperatureUnit.CELSIUS);
	    Quantity<TemperatureUnit> result = t.convertTo(TemperatureUnit.CELSIUS);
	    assertEquals(50.0, result.convertToBaseUnit(), EPSILON);
	}

	@Test
	void testTemperatureConversion_RoundTrip() {
	    Quantity<TemperatureUnit> t = new Quantity<>(37.0, TemperatureUnit.CELSIUS);
	    Quantity<TemperatureUnit> result = t.convertTo(TemperatureUnit.FAHRENHEIT)
	                                        .convertTo(TemperatureUnit.CELSIUS);
	    assertEquals(37.0, result.convertToBaseUnit(), EPSILON);
	}
	...
	@Test
	void testOperationSupport_TemperatureReturnsFalse() {
	    assertFalse(TemperatureUnit.CELSIUS.supportsArithmetic());
	    assertFalse(TemperatureUnit.FAHRENHEIT.supportsArithmetic());
	}

	@Test
	void testOperationSupport_LengthReturnsTrue() {
	    assertTrue(LengthUnit.FEET.supportsArithmetic());
	}

	@Test
	void testOperationSupport_WeightReturnsTrue() {
	    assertTrue(WeightUnit.KILOGRAM.supportsArithmetic());
	}

	@Test
	void testOperationSupport_VolumeReturnsTrue() {
